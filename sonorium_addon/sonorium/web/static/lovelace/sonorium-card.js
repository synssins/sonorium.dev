 ...
 *
 * Installation:
 * 1. Copy this file to your Home Assistant www folder (e.g., /config/www/sonorium-card.js)
 * 2. Add the resource in Settings > Dashboards > Resources:
 *    URL: /local/sonorium-card.js
...
class SonoriumCard extends HTMLElement {
    ...
    setConfig(config) {
        if (!config.sonorium_url) {
            throw new Error('Please define sonorium_url');
        }
        this._config = {
            title: config.title || 'Sonorium',
            sonorium_url: config.sonorium_url.replace(/\/$/, ''), // Remove trailing slash
    ...
    set hass(hass) {
        this._hass = hass;
        // Could use hass for speaker entity states in the future
    }
    ...
    static getStubConfig() {
        return {
            title: 'Sonorium',
            sonorium_url: 'http://homeassistant.local:8009',
            show_channels: true,
            show_themes: true,
            compact: false
        };
    ...
    _render() {
        this.innerHTML = `
            ...
            <div class="editor">
                ...
                <div class="form-row">
                    <label for="sonorium_url">Sonorium URL</label>
                    <input type="text" id="sonorium_url" value="${this._config.sonorium_url || ''}" placeholder="http://homeassistant.local:8009" />
                    <span class="helper-text">The URL to your Sonorium instance (check addon config for port)</span>
                </div>
                <div class="form-row">
